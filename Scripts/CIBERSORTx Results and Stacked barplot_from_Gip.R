...
CIBERSORTx_results <- read.delim("Original_data/CIBERSORTx_Job11_Adjusted.txt") |> 
  dplyr::select(1:17)
...
CIBERSORTx_percent <- CIBERSORTx_results  |> 
  dplyr::pivot_longer(-Mixture, names_to = "Cell_stage", values_to = "value") |> 
  dplyr::group_by(Mixture, Cell_stage) |> 
  dplyr::summarize(sum_value = sum(value)) |> 
  dplyr::group_by(Mixture) |> 
  dplyr::mutate(percent = sum_value / sum(sum_value) * 100) |> 
  dplyr::ungroup() |> 
