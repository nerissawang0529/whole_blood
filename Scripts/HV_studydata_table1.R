...
studydata <- studydata %>% filter(group %in% c("HV"))
...
studydata[, c("pathogen", "Gram_negative", "Gram_positive", "Virus", "Fungi")] <- NA
## table
...
tab1_df <- as.data.frame(print(tab2, nonnormal = nonnormal, quote = FALSE, noSpaces = TRUE, smd = TRUE, missing = TRUE))
...
export_file_name <- "table1_hv.csv" 
...
##
studydata_HV <- studydata[,c("EB_id","group","age_yrs",	"gender",	"ethnic_group#White",	
                              "BMI",	"symptom_days",
                              "CCI",	"hypertension",	"cpd",	"COPD",	"diabetes",	
                              "ccd","ckd",	"mneoplasm",	"immune_sup",	"cnd",	
                              "qSOFA_score",	"MEWS_score",	"CURB_score",	"PSI_new",
...
##export form
destination_folder <- "Original_data/" 
export_file_name <- "studydata_HV.csv" 
write.csv(studydata_HV, file.path(destination_folder, export_file_name), row.names = FALSE)
cat("File exported to:", file.path(destination_folder, export_file_name), "\n")
