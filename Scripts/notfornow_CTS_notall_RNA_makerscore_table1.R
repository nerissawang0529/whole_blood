...
# If you're reading from CSV:
#the following dataframe come from the code 'SRSq', and the dataframe called 'expr_counts' in the code
cnt <- read.csv("Original_data/Combined_counts_Amsterdam_my_cohort.csv", check.names = FALSE)

## Assume you already have cnt loaded like in the screenshot
# If reading from CSV: cnt <- read.csv("Combined_counts_Amsterdam.csv", check.names = FALSE)
...
dfwb_sel <- read.csv("Original_Data/dfwb_sel.csv", check.names = FALSE)
library(dplyr)
...
#table one for CTS
studydata <-read.csv("Original_Data/studydata.csv", check.names = FALSE)
...
allvars <- c("group","flow_Data", "spectral", "inclusion_hospital", "sampling_time",		"age_yrs",	"gender",	"ethnic_group#White",	
             ...
             "hospdeath",	"mortality_d30", "mortality_d90","CTS")
