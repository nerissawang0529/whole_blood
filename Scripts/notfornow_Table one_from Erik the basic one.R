...
studydata <- import("L:/basic/Personal Archive/H/hwang/CAP/ELDER-BIOME_excel_export_20230418012733.xlsx")
## p_load(tableone)

##
list_of_patients <- readxl::read_xlsx("L:/basic/Personal Archive/H/hwang/CAP/2304_Present WB stimulation list EB V2.xlsx")
table(list_of_patients$Stimulations)

##
studydata <- studydata[studydata$`Participant Id` %in% list_of_patients$`Patient study ID`, ]
...
studydata$base_oxygen[is.na(studydata$base_oxygen) & !is.na(studydata$base_spo2)] <- 0

studydata$MEWS_sat_ther <- with(studydata, ifelse(base_oxygen >= 1 & base_spo2 < 90, 3, 0))
                                                ...
                                                ifelse(base_syst_bp <= 70, 3,
                                                       ifelse(base_syst_bp %in% 71:80, 2, 
                                                              ifelse(base_syst_bp %in% 81:100, 1,
                                                                     ifelse(base_syst_bp %in% 101:200, 0,
                                                                            ifelse(base_syst_bp >= 200, 2, NA)))))))
studydata$MEWS_temp <- with(studydata, ifelse(is.na(base_temp), NA,
                                              ifelse(base_temp < 35.1, 2, 
                                                     ifelse(base_temp >= 35.1 & base_temp <= 36.5, 1,
                                                            ifelse(base_temp >= 36.6 & base_temp <= 37.5, 0,
                                                                   ifelse(base_temp > 37.5, 1, NA))))))

studydata$base_mental_status 
studydata$MEWS_conciousness <- ifelse(!is.na(studydata$base_mental_status) & studydata$base_mental_status == "No", 1, 0)
...
## FLOW data
TOM_flow_data <- import("L:/basic/Personal Archive/H/hwang/Gating/Date from Tom's paper/OMIQ_metadata-flow1_metadata name.csv")
pneumonia$flow_Data <- ifelse(pneumonia$EB_id %in% TOM_flow_data$cap_covid_hv, 1, 0)

## spectral flow COVID
ERIK_spectral <- import("L:/basic/Personal Archive/H/hwang/CAP/spectral data superinfection.xlsx")
pneumonia$spectral <- ifelse(pneumonia$EB_id %in% ERIK_spectral$EB_ID, 1, 0)

## 
## analyse errors 
allvars <- c("flow_Data", "spectral", "inclusion_hospital", "sampling_time",		"age_yrs",	"gender",	"ethnic_group#White",	
             "BMI",	"symptom_days",
...
catvars <- c("flow_Data", "spectral", "inclusion_hospital", "gender",	"ethnic_group#White",	
...
print(tab1, nonnormal = nonnormal, quote = TRUE, noSpaces = TRUE, smd = T, missing = T)
