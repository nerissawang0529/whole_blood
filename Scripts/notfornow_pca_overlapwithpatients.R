...
ldata_sel <- ldata[ldata$stimulation %in% c("PP","LPS","KP") &
                     ldata$variable %in% c("TNF","IL_1RA"),]

ldata_wide_ss <- reshape2::dcast(ldata_sel, ID ~ variable*stimulation, value.var = "level")
...
ldata_HV_sel <- ldata_HV[ldata_HV$stimulation %in% c("PP","LPS","KP") &
                     ldata_HV$variable %in% c("TNF","IL_1RA"),]

ldata_HV_wide_ss <- reshape2::dcast(ldata_HV_sel, ID ~ variable*stimulation, value.var = "level")
...
label_offset <- 1.1
label_size <- 5
marker_colors <- c("IL-1RA" = "#4DAF4A",  "TNF" = "#FF7F00")
...
### centroid and sds of HV

HV_mu <- colMeans(pca_HV_df[,1:2])
HV_sd <- apply(pca_HV_df[,1:2], 2, sd)


z_all <- pca_all
z_all$z1 <- (z_all$PC1 - HV_mu[1])/HV_sd[1]
z_all$z2 <- (z_all$PC2 - HV_mu[2])/HV_sd[2]

boxplot(z_all$z1 ~ z_all$group)
boxplot(z_all$z2 ~ z_all$group)

### euclidean distance from HV centroid using Pythagoras equation
z_all$z_hyp <- sqrt( z_all$z1^2 + z_all$z2^2 )


ggplot(z_all, aes(PC1, PC2,  shape = group)) +
  geom_point(aes(fill = z_hyp),size = 2, alpha = 0.85, shape=21) +
  stat_ellipse(aes(group = cluster, color=group), linewidth = 1.2) +
  geom_segment(data = loadings_df,
               aes(x = 0, y = 0,
                   xend = PC1 * arrow_scale,
                   yend = PC2 * arrow_scale,
...
### healthy cutoff try z = 1.65

z_all$HV_like <- z_all$z_hyp < 1.96

ggplot(z_all, aes(PC1, PC2,  shape = group)) +
  geom_point(aes(fill = HV_like, shape=group),size = 2, alpha = 0.85) +
  stat_ellipse(aes(group = cluster, color=group), linewidth = 1.2) +
  geom_segment(data = loadings_df,
               aes(x = 0, y = 0,
                   xend = PC1 * arrow_scale,
...
table(z_all$group, z_all$HV_like)
