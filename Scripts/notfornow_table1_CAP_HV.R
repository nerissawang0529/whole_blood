...
#studydata data for CAP & HV
studydata <- studydata %>%
  filter(!EB_id %in% notCOVID_notCAP$EB_id)
studydata <- studydata %>% filter(group %in% c("CAP", "HV"))
...
## FLOW data
TOM_flow_data <- read.csv("Documents/Luminex/R_code/Original_data/OMIQ_metadata-flow1_metadata name.csv")
...
export_file_name <- "studydata_CAP_HV.csv" 
...
#export clinical_marker_unique data
destination_folder <- "Documents/Luminex/R_code/Original_data/" 
export_file_name <- "table1_CAP_HV.csv" 
...
studydata$pathogen <- ifelse(
  studydata$EB_id %in% c(1236, 1243, 3321, 3332, 3333, 3334, 3336, 3340, 3348), "unknown",
...
#these 9 patients are cap but didn't tested for bacterial

table(studydata$pathogen)

no_pathogen_information <- studydata[,c("EB_id","other_pathogen","outcome_pat_posthoc","outcome_pat_posthoc_other","outcome_pat_other_diagnosis")]
...
allvars <- c("inclusion_hospital", "sampling_time",		"age_yrs",	"gender",	"ethnic_group#White",	
             ...
             "Non_invasive_ventilation_1",	"Invasive_ventilation_1",	"lenght_of_intubation",
             "bacteremia", "pathogen_cultured",
             "hospdeath",	"mortality_d30", "mortality_d90","pathogen")
...
catvars <- c("inclusion_hospital", "gender",	"ethnic_group#White",	
             ...
             "Non_invasive_ventilation_1",	"Invasive_ventilation_1",	
             "bacteremia",
             "hospdeath",	"mortality_d30", "mortality_d90", "pathogen_cultured","pathogen")
...
##export form
destination_folder <- "Documents/Luminex/R_code/Original_data/" 
export_file_name <- "studydata_CAP.csv" 
write.csv(studydata, file.path(destination_folder, export_file_name), row.names = FALSE)
cat("File exported to:", file.path(destination_folder, export_file_name), "\n")
...
#export clinical_marker_unique data
destination_folder <- "Documents/Luminex/R_code/Original_data/" 
export_file_name <- "table1_CAP_pathgon.csv" 
write.csv(tab2_df, file.path(destination_folder, export_file_name), row.names = FALSE)
cat("File exported to:", file.path(destination_folder, export_file_name), "\n")


##
studydata_pathogen <- studydata[,c("EB_id","outcome_pat_cause1","outcome_pat_cause2","outcome_pat_cause3",
                                   "mixed_infections","no_pathogen","strep","aureus","influenza","covid","h_influenz","other_pathogen","pathogen")]
