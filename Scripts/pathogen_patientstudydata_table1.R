...
#there are some patients are not CAP and COVID-19
...
#exclude the not COVID-19 and not CAP (number = 14)
...
export_file_name <- "notCOVID_notCAP.csv" 
write.csv(notCOVID_notCAP, file.path(destination_folder, export_file_name), row.names = FALSE)
...
studydata <- studydata %>% filter(group %in% c("CAP"))
...
studydata$influenza <- ifelse(studydata$outcome_pat_cause1 == "Influenza A virus", 1, 
                              ifelse(studydata$outcome_pat_cause2 == "Influenza A virus", 1, 
                                     ifelse(studydata$outcome_pat_cause3 == "Influenza A virus", 1, 0))) #Influenza B virus?
...
#3354 can not be found by "No causative agent found"
studydata$no_pathogen <- ifelse(studydata$outcome_pat_cause1 == "No causative agent found" | studydata$EB_id == 3354, 1, 0)

##
studydata$outcome_pat_cause2[studydata$EB_id == 3048] <- NA # for 3048, outcome_pat_cause1 and 2 are same, both are Rhinovirus.

studydata$mixed_infections <- ifelse(!is.na(studydata$outcome_pat_cause2), 1, 0)
studydata$mixed_infections[studydata$EB_id == 3330] <- 1
studydata$outcome_pat_cause1[studydata$EB_id == 3330] <- "Streptococcus intermedius"
studydata$outcome_pat_cause2[studydata$EB_id == 3330] <- "Lactobacillus casei"
#3330 is Streptococcus intermedius & Lactobacillus casei
...
studydata$other_pathogen[studydata$EB_id == 3322] <- 1
studydata$outcome_pat_cause1[studydata$EB_id == 3322] <- "Respiratory syncytial virus (RSV)"
#3322, Respiratory syncytial virus (RSV)

#Aspergillus spp,Coronavirus,Escherichia coli, human MetaPneumoVirus (hMPV) ,Mycobacterium tuberculosis,Other,Parainfluenza virus 1-4
#Pneumocystis carinii,Pseudomonas aeruginosa,Respiratory syncytial virus (RSV),Rhinovirus,Streptococcus pyogenes
...
studydata$other_pathogen[studydata$EB_id == 3335] <- "0"
studydata$strep[studydata$EB_id == 3335] <- "1"

##
# Assign "untested" to specific EB_id values
studydata$pathogen <- ifelse(
  studydata$EB_id %in% c(1236, 1243, 3321, 3332, 3333, 3334, 3336, 3340, 3348), "untested", #these 9 patients are cap but didn't tested for bacterial
  ifelse(studydata$EB_id == 3322 & studydata$outcome_pat_cause1 == "Respiratory syncytial virus (RSV)", "other",
         ifelse(studydata$mixed_infections == 1, "mixed_infection",
                ifelse(studydata$no_pathogen == 1, "pathogen_negative",
...
#not taken into account for now about the Fungi and Virus, but i list them in the dataframe
##
...
studydata$Gram_negative <- ifelse(studydata$outcome_pat_cause1 ==  "Escherichia coli" | studydata$outcome_pat_cause1 == "Haemophilus influenzae" |  studydata$outcome_pat_cause1 == "Pseudomonas aeruginosa" | studydata$outcome_pat_cause1 == "Stenotrophomonas maltophilia"   | studydata$outcome_pat_cause1 == "Moraxella osloensis"  , 1, 0)
studydata$Gram_positive <- ifelse(studydata$outcome_pat_cause1 ==  "Rothia dentocariosa" | studydata$outcome_pat_cause1 ==  "Staphylococcus aureus" | studydata$outcome_pat_cause1 ==  "Streptococcus pneumoniae" 
                                  | studydata$outcome_pat_cause1 ==  "Streptococcus pyogenes" | studydata$outcome_pat_cause1 ==  "Streptococcus intermedius"| studydata$outcome_pat_cause1 ==  "Streptococcus salivarius", 1, 0)
...
studydata$Gram_negative <- ifelse(studydata$outcome_def_pathogen2 == "Moraxella osloensis" | studydata$outcome_def_pathogen2 == "Serratia marcescens"| 
                                    studydata$outcome_def_pathogen1 == "Stenotrophomonas maltophilia"| studydata$outcome_def_pathogen1 == "Pseudomonas fluorescens"
                                  | studydata$outcome_def_pathogen1 == "Moraxella catarrhalis", 1, studydata$Gram_negative)
...
studydata$Gram_positive[studydata$EB_id == 3335] <- "1"

##
one_normal_pathogen <- filter(studydata, other_pathogen != 1 | is.na(other_pathogen))
one_normal_pathogen <- filter(one_normal_pathogen, !one_normal_pathogen$mixed_infections == 1)
one_normal_pathogen <- dplyr::select(one_normal_pathogen, "EB_id","outcome_pat_cause1", "outcome_def_pathogen1",
                                     "Gram_negative","Gram_positive","Virus","Fungi")
#export 
destination_folder <- "Original_data/" 
export_file_name <- "one_normal_pathogen.csv" 
write.csv(one_normal_pathogen, file.path(destination_folder, export_file_name), row.names = FALSE)
cat("File exported to:", file.path(destination_folder, export_file_name), "\n")

##
other_pathogens <- filter(studydata, studydata$other_pathogen == 1)
other_pathogens <- dplyr::select(other_pathogens, "EB_id","outcome_pat_cause1", "outcome_def_pathogen1",
                                 "Gram_negative","Gram_positive","Virus","Fungi")
#export 
destination_folder <- "Original_data/" 
export_file_name <- "other_pathogens.csv" 
write.csv(other_pathogens, file.path(destination_folder, export_file_name), row.names = FALSE)
cat("File exported to:", file.path(destination_folder, export_file_name), "\n")


##
mixed_infection <- filter(studydata, studydata$mixed_infections == 1)
mixed_infection_short <- dplyr::select(mixed_infection, "EB_id", "outcome_pat_cause1", "outcome_pat_cause2", "outcome_def_pathogen2", "outcome_pat_cause3", 
                                       "outcome_def_pathogen3","Gram_negative","Gram_positive","Virus","Fungi")
#export 
destination_folder <- "Original_data/" 
export_file_name <- "mixed_infection_short.csv" 
write.csv(mixed_infection_short, file.path(destination_folder, export_file_name), row.names = FALSE)
cat("File exported to:", file.path(destination_folder, export_file_name), "\n")
...
ICU_and_sampling_same_day <-  dplyr::select(ICU_and_sampling_same_day, "EB_id", "base_date_assessment", "outcome_pat_icu_admission", "time_sampling_ICU_admission")
...
###respiratory rate for MEWS score 
...
## FLOW data
TOM_flow_data <- read.csv("Original_data/OMIQ_metadata-flow1_metadata name.csv")
studydata$flow_Data <- ifelse(studydata$EB_id %in% TOM_flow_data$cap_covid_hv, 1, 0)
#need to check the column names

## spectral flow COVID
ERIK_spectral <- import("Original_data/spectral data superinfection_from Erik_the COVID-19 patients.xlsx")
studydata$spectral <- ifelse(studydata$EB_id %in% ERIK_spectral$EB_ID, 1, 0)
...
allvars <- c("inclusion_hospital", "sampling_time",		"age_yrs",	"gender",	"ethnic_group#White",	
             ...
             "hospdeath",	"mortality_d30", "mortality_d90","pathogen","Gram_negative","Gram_positive","Virus","Fungi")
...
#export clinical_marker_unique data
destination_folder <- "Original_data/" 
export_file_name <- "table1_cap.csv" 
...
##
studydata_patients <- studydata[,c("EB_id","group","age_yrs",	"gender",	"ethnic_group#White",	
                                   "BMI",	"symptom_days",
                                   "CCI",	"hypertension",	"cpd",	"COPD",	"diabetes",	
                                   "ccd","ckd",	"mneoplasm",	"immune_sup",	"cnd",	
                                   "qSOFA_score",	"MEWS_score",	"CURB_score",	"PSI_new",
...
##export form
destination_folder <- "Original_data/" 
export_file_name <- "studydata_patients.csv" 
write.csv(studydata_patients, file.path(destination_folder, export_file_name), row.names = FALSE)
cat("File exported to:", file.path(destination_folder, export_file_name), "\n")
